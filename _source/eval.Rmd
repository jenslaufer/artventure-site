...
dataWithPriceIncrease %>%
  ...
  geom_boxplot() +
  scale_x_discrete(guide = guide_axis(angle = 45), limit=dataWithPriceIncrease %>% filter(class %in% interesting_classes) %>% group_by(class) %>% summarise(median=median(price_increase_pct, na.rm = T)) %>% arrange(-median) %>% pull(class)) + 

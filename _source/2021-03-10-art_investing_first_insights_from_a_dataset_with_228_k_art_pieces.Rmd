...
artist_data <- data %>%
  ...
  group_by(artist) %>% 
  summarise(total_price_increase = sum(price_increase)) %>% 
  arrange(-total_price_increase) %>% 
  head(30) 

artist_plot <- artist_data %>% 
  ggplot(aes(x=total_price_increase, y = artist)) +
  geom_lollipop(stat = "identity", horizontal = T, point.colour = "#4e79a7", point.size = 2) +
  scale_y_discrete(limit=artist_data %>% arrange(total_price_increase) %>% pull(artist)) +
  bbc_style() +
...
By adding up all absolute price increases for all the artists I want to get insights which artists have the highest total price increase.

From the chart you can see Mr. Brainwash, Space Invader, KAWS and Shepard Fairey are on top of the list.

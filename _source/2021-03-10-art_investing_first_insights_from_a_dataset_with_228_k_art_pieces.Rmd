...
### What are the most common art pieces are in the data set?
...
art_classes %>%
  distinct(class2, ratio2) %>%
  mutate(ratio2 = ratio2 * 100) %>%
  rename(class = class2, ratio = ratio2) %>%
  ggplot(aes(
    x = '',
    y = ratio,
    fill = factor(
  ...
  geom_text(aes(label=paste0(round(ratio,1),"% (",num,")"))) +

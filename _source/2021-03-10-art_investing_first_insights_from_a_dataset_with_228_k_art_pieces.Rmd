...
#valueBox
valueBox %>% 
  finalise_plot(source_name = "Source: https://artventure.me Data Source: https://expressobeans.com", save_filepath = "overall_overview.jpeg")
```
...
### Most valuable artists in the data set


```{r}
data %>% 
  drop_na(original_price, lowest_price, highest_price, average_price) %>% 
  mutate(average_price_increase=(average_price-original_price)/original_price) %>% 
  group_by(artist) %>% 
  summarise(n=n(), average_price_increase_per_artpiece=average_price_increase/n()) %>% 
  arrange(-average_price_per_piece)
```
